...
Нажмите **Win+X** → **Windows PowerShell** (или **Терминал Windows**).

### Шаг 2 — Вставьте и выполните эту команду (ОДНА строка, копировать целиком)

```powershell
Set-ExecutionPolicy -Scope Process Bypass; git clone https://github.com/ybiytsa1983-cpu/drgr-bot.git "$env:USERPROFILE\Desktop\drgr-bot"; Set-Location "$env:USERPROFILE\Desktop\drgr-bot"; pip install -r requirements.txt; Write-Host "Готово! Запустите ЗАПУСТИТЬ_БОТА.bat" -ForegroundColor Green
```

> После этого на **Рабочем столе** появится папка `drgr-bot`.  
> Укажите токен бота в файле `.env`, затем дважды кликните **`ЗАПУСТИТЬ_БОТА.bat`**.
...
**Способ 1 — BAT-файл (работает сразу, всегда есть в папке):**

```powershell
Start-Process "$env:USERPROFILE\Desktop\drgr-bot\ЗАПУСТИТЬ_БОТА.bat"
```

Или просто дважды кликните **`ЗАПУСТИТЬ_БОТА.bat`** на Рабочем столе.

**Способ 2 — PowerShell-скрипт `run.ps1` (появится после обновления):**

Сначала обновитесь, потом запускайте:

```powershell
Set-ExecutionPolicy -Scope Process Bypass
cd "$env:USERPROFILE\Desktop\drgr-bot"
