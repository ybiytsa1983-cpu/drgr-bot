...
## 🔌 Chrome расширение (DRGR VM Connector)

Расширение — быстрый доступ к статусу VM прямо из панели Chrome.

### Установка расширения

1. Запустите **Code VM.lnk** (или `start_vm.ps1`) — сервер стартует, окно подскажет путь.
...
5. Выберите папку `extension` внутри папки проекта (`Desktop\drgr-bot\extension`).
6. Значок расширения появится на панели инструментов браузера.

> 💡 Расширение работает только пока запущен сервер (`start_vm.ps1` / `Code VM.lnk`).  
> Без сервера используйте веб-интерфейс напрямую: **http://localhost:5000**
...
**Code VM.lnk не запускается / окно мгновенно закрывается**
→ Запустите `start_vm.ps1` напрямую через PowerShell — окно останется открытым и покажет ошибку.
→ Убедитесь, что Python установлен и в PATH (`python --version` в PowerShell).

**Chrome расширение не работает / не подключается**
→ Убедитесь, что сервер запущен (`Code VM.lnk` или `start_vm.ps1`).
→ Проверьте, что расширение загружено через `chrome://extensions` (см. раздел «Chrome расширение»).
→ Если всё установлено, откройте веб-интерфейс напрямую: **http://localhost:5000**
