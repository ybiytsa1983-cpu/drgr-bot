...
Рекомендованный способ — **дважды щёлкните `ЗАПУСТИТЬ.bat`** в папке с ботом.

Или через PowerShell (терминал открыт в папке бота):

```powershell
# Вариант 1 — ASCII-имя, работает на любой Windows
.\run.ps1

# Вариант 2 — кириллическое имя (если оно корректно отобразилось при клонировании)
.\ЗАПУСТИТЬ.ps1
```
...
| `ЗАПУСТИТЬ.ps1` не найден / кириллика сломалась | Используйте ASCII-вариант: `.\run.ps1` — он есть в папке бота |
| PowerShell жалуется «-File не существует» | Используйте `.\run.ps1` или просто запустите `ЗАПУСТИТЬ.bat` двойным щелчком |
