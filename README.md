...
```powershell
Set-ExecutionPolicy -Scope Process Bypass; [Net.ServicePointManager]::SecurityProtocol = [Net.SecurityProtocolType]::Tls12; $f="$env:TEMP\install_drgr.bat"; Invoke-WebRequest -Uri "https://raw.githubusercontent.com/ybiytsa1983-cpu/drgr-bot/main/%D0%A3%D0%A1%D0%A2%D0%90%D0%9D%D0%9E%D0%92%D0%98%D0%A2%D0%AC.bat" -OutFile $f -UseBasicParsing; if (Test-Path $f) { cmd /c "`"$f`"" } else { Write-Host "Ошибка: файл не скачался. Проверьте интернет." -ForegroundColor Red }
```
...
if (Test-Path $f) { cmd /c "`"$f`"" } else { Write-Host "Ошибка: файл не скачался. Проверьте интернет." -ForegroundColor Red }
```
