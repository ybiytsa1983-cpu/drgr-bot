...
`python` и локальный `javascript/typescript` run в этом server-шаблоне помечены как disabled (безопасный lightweight режим).  
Для реального выполнения кода подключите отдельный изолированный раннер/контейнер (sandbox service) и проксируйте выполнение через `/api/editor/run`.
